"""
App model with Mixin pattern for better organization.

All code remains in a single file, but functionality is organized into logical mixin classes:
- LifecycleMixin: create, delete, restart, save
- DeployMixin: deploy, scale, mount, pipeline, clean
- KubernetesMixin: pod/deployment/event operations
- HealthMixin: HTTP/TCP health verification
- ConfigMixin: configuration and environment management
- AutoscaleMixin: HPA autoscaling
- UsageMixin: usage reporting
- StateMixin: state synchronization
"""
...
def validate_app_id(value):
    ...
    if not match:
        raise ValidationError(
            "App name must start with an alphabetic character, "
            "cannot end with a hyphen and can only contain "
            "a-z (lowercase), 0-9 and hyphens."
        )
...
class AppBaseMixin:
    """Base mixin providing common utilities for all App mixins.

    All other App mixins should inherit from this class to access:
    - lock: distributed app lock
    - log: application logging
    - ptypes: process type property
    - check_ptypes: validate process types
    - get_deploy_name: K8s deployment name generation
    - get_system_actor: system user for automated operations
    """

    @property
    def ptypes(self):
        """List of process types from app structure."""
        return list(self.structure.keys())

    def log(self, message, level=logging.INFO):
        """Log a message in the context of this application."""
        send_app_log.delay(self.id, message, level)
        logger.log(level, "[{}]: {}".format(self.id, message))

    def lock(self):
        """Acquire distributed lock for this app."""
        ...
        if not ptypes:
            ...
            if len(invalid_ptypes) != 0:
                raise DryccException(
                    "process type {} is not included in procfile".format(",".join(invalid_ptypes))
                )
        return ptypes

    def get_deploy_name(self, ptype):
        """Get Kubernetes deployment name for a process type."""
        return f"{self.id}-{ptype}"

    def get_system_actor(self):
        """Get system actor for automated operations (not a real user)."""
        return namedtuple("SystemActor", ["username"])("system")


class LifecycleMixin(AppBaseMixin):
    """Mixin for App lifecycle operations: create, restart."""

    def create(self, *args, **kwargs):
        """
        Create application with initial config, settings, release, domain and k8s resources.
        """
        cfg = self._set_default_config()
        try:
            self.release_set.latest()
        except Release.DoesNotExist:
            Release.objects.create(version=1, app=self, config=cfg, build=None)
        ...
        try:
            ...
            AppSettings.objects.create(app=self, routable=True, autodeploy=True, autorollback=True)
        try:
            self.tls_set.latest()
    ...
    def restart(self, **kwargs):
        ...
        if self.structure[kwargs["type"]] > 0:
            deployments.append(self.get_deploy_name(kwargs["type"]))
        try:
            tasks = [
                (
                    functools.partial(self.scheduler.deployment.restart, self.id, deployment),
    ...
    def _create_network_policy(self):
        """Create network policy for the app namespace."""
        if not settings.DRYCC_NETWORK_POLICY_TEMPLATE:
            return
        name = namespace = self.id
        json_str = (
            Template(settings.DRYCC_NETWORK_POLICY_TEMPLATE)
            .render(Context({"workspace": str(self.workspace.id)}))
            .strip()
        )
            ...
            if e.response.status_code == 404:
                try:
                    ...
                    raise ServiceUnavailable(
                        "Could not create the NetworkPolicy in Kubernetes"
                    ) from ex
            else:
                raise ServiceUnavailable("Could not get the NetworkPolicy in Kubernetes") from e
...
class DeployMixin(AppBaseMixin):
    """Mixin for deploy operations: deploy, scale, mount, pipeline, clean."""
    ...
    def deploy(self, release, ptypes=None, force_deploy=False, rollback_on_failure=True):
        ...
        for scale_type, replicas in self.structure.items():
            ...
            deploys[scale_type] = self.gather_app_settings(
                release, app_settings, scale_type, replicas, volumes=scale_type_volumes
            )
        self._deploy(deploys, ptypes, prev_release, release, force_deploy, rollback_on_failure)
    ...
    def pipeline(self, release, ptypes, force_deploy=False):
        """Execute pipeline for a release."""
            ...
            if release is not None and release.build is not None:
                if release.build.dryccfile:
                    for run in release.get_runners(ptypes):
                        ...
                        job_name = self.run(
                            self.get_system_actor(),
                            run["image"],
                            command=run["command"],
                            args=run["args"],
                            timeout=run["timeout"],
                            expires=run["timeout"],
                            envs=self.build_env_vars(release, run["ptype"]),
                        )
                        state, labels = "initializing", {"job-name": job_name}
                        for count, state in enumerate(
                            self.scheduler.pod.watch(
            ...
            for ptype in pre_release.ptypes:
                if ptype not in release.ptypes and self.structure.get(ptype, 0) > 0:
                    if ptypes is None or ptype in ptypes:
                        removed[ptype] = 0
            self.scale(self.get_system_actor(), removed)
    ...
    def _deploy(self, deploys, ptypes, prev_release, release, force_deploy, rollback_on_failure):
                ...
                tasks.append(
                    (
                        functools.partial(
                            ...
                            namespace=self.id,
                            name=self.get_deploy_name(scale_type),
                        ...
                        lambda future: self.log(
                            "deploy and unlock callback: "
                            f"{[future.result(), lock.release([scale_type])]}"
                        ),
                    )
                )
            try:
                ...
                if (
                    ...
                    and prev_release.build is not None
                ):
        ...
        for ptype in deploys.keys():
            ...
            if prev_release and prev_release.build:
                continue
            if ptype == PTYPE_WEB:
                self.verify_http_health(service, **deploys[ptype])
            else:
                self.verify_tcp_health(service, **deploys[ptype])

    def _scale(self, user, structure, release, app_settings):
        ...
        if new_scale:
            ...
            App.objects.filter(id=self.id).update(
                structure=Func(
                    F("structure"), Value(new_scale, JSONField()), function="jsonb_concat"
                )
            )
    ...
    def _scale_pods(self, scale_types, release, app_settings):
        """Scale pods for given process types."""
        volumes = Volume.objects.filter(app=self).exclude(path={})
        tasks = []
        for scale_type, replicas in scale_types.items():
            scale_type_volumes = [volume for volume in volumes if scale_type in volume.path.keys()]
            data = self.gather_app_settings(
                release, app_settings, scale_type, replicas, volumes=scale_type_volumes
            )
            self.set_application_config(release, scale_type)
            tasks.append(
        ...
        for scale_type, replicas in structure.items() if structure else self.structure.items():
            if scale_type != PTYPE_RUN:
                release = self.release_set.filter(
                    deployed_ptypes__contains=scale_type, failed=False
                ).latest()
    ...
    def _merge_structure(self, release, prev_release):
        ...
        lock = self.lock()
        if not lock.acquire():
            raise ServiceUnavailable(f"could not acquire app lock for {self.id}")
        try:
            ...
            if (self.structure != default_structure) or (
                prev_release
                and prev_release.build
                and prev_release.build.type != release.build.type
            ):
                ...
                # Use QuerySet.update() instead of self.save() to avoid triggering
                # App.save() side effects (self.create() which provisions k8s resources).
                # We only need to persist the structure field to the database here.
                App.objects.filter(id=self.id).update(structure=default_structure)
        finally:
            lock.release()
    ...
    def _check_deployment_in_progress(self, deploys, force_deploy=False):
        ...
        for scale_type, kwargs in deploys.items():
            ...
            in_progress, deploy_okay = self.scheduler.deployment.in_progress(
                self.id,
                name,
                kwargs.get("deploy_timeout"),
                kwargs.get("deploy_batches"),
                kwargs.get("replicas"),
                kwargs.get("tags"),
            )
    ...
    def _create_default_ingress(self, target_port):
        """Create default ingress resources (service, gateway, route)."""
        try:
...
class RunMixin(AppBaseMixin):
    """Mixin for one-off command execution."""
...
class KubernetesMixin(AppBaseMixin):
    """Mixin for K8s resource operations: pods, deployments, events."""
    ...
    def list_pods(self, *args, **kwargs):
        ...
        try:
            ...
            if "name" in kwargs:
                ...
                pods = self.scheduler.pod.get(self.id, labels=labels).json()["items"] or []
            data = []
            for p in pods:
                ...
                item = {
                    ...
                    "type": labels["type"],
                    "started": started,
                    "ready": "%s/%s" % (ready, len(p["spec"]["containers"])),
                    "restarts": restarts,
                }
                data.append(item)
    ...
    def list_deployments(self, *args, **kwargs):
        ...
        try:
            ...
            if "name" in kwargs:
                ...
                deployments = (
                    self.scheduler.deployment.get(self.id, labels=labels).json()["items"] or []
                )
            data = []
            for p in deployments:
                ...
                item = {
                    ...
                    "release": labels["version"],
                    "ready": "%s/%s"
                    % (p["status"].get("readyReplicas", 0), p["spec"].get("replicas", 0)),
    ...
    def list_events(self, ref_kind, ref_name, *args, **kwargs):
        """List events for this app."""
        try:
            fields = {"regarding.kind": ref_kind, "regarding.name": ref_name}
            kwargs["fields"] = fields
            events = self.scheduler.events.get(self.id, **kwargs).json()["items"]
            data = []
            for e in events:
                item = {
                    "reason": e["reason"],
...
class HealthMixin(AppBaseMixin):
    """Mixin for health check operations."""

    def verify_http_health(self, service, **kwargs):
        """Verify HTTP service health."""
        ...
        if (
            "livenessProbe" in kwargs.get("healthcheck", {})
            and "httpGet" in kwargs["healthcheck"]["livenessProbe"]
        ):
            allowed = [200]
        ...
        if (response and response.status_code == 404) or failed:
            ...
            self.log(
                "Router was not ready to serve traffic "
                "to process type {} in time, waited {} seconds".format(
                    app_type, delta
                ),
    ...
    def verify_tcp_health(self, service, **kwargs):
        """Verify TCP service health."""
        for _ in range(10):
...
class ConfigMixin(AppBaseMixin):
    """Mixin for configuration and environment management."""

    def set_application_config(self, release, ptype):
        """Create/update application config as a Kubernetes secret."""
        labels = {"version": release.version_name, "type": ptype, "class": "env"}
        secrets_env = {}
        for key, value in self.build_env_vars(release, ptype).items():
    ...
    def image_pull_secret(self, namespace, ptype, registry, image):
        """Set imagePullSecret for registry."""
        ...
        if create is None:
            ...
            try:
                ...
                self.scheduler.secret.create(
                    namespace, name, data, secret_type="kubernetes.io/dockerconfigjson"
                )
            else:
    ...
    def build_env_vars(self, release, ptype):
        ...
        default_env = {
            ...
            "WORKFLOW_RELEASE_CREATED_AT": str(
                release.created.strftime(settings.DRYCC_DATETIME_FORMAT)
            ),
            "SOURCE_VERSION": release.build.sha,
        }
    ...
    def _get_private_registry_config(self, ptype, image, registry=None):
        """Get private registry configuration."""
        ...
        if registry:
            ...
            secret = self.scheduler.secret.get(
                settings.WORKFLOW_NAMESPACE, "controller-creds"
            ).json()
        ...
        docker_config = json.dumps(
            {"auths": {hostname: {"auth": base64.b64encode(auth).decode(encoding="UTF-8")}}}
        )
        return docker_config, name, True

    def _set_default_limit(self, config, ptype):
        """Set default limit for a process type."""
    ...
    def _get_volumes_and_mounts(self, ptype, volumes):
        """Get Kubernetes volume configurations."""
    ...
    def gather_app_settings(self, release, app_settings, ptype, replicas, volumes=None):
        """Gather all required information for deploying an application."""
        envs = self.build_env_vars(release, ptype)
        ...
        termination_grace_period_seconds = int(
            config.termination_grace_period.get(
                ptype,
                envs.get(
        ...
        routable = True if (ptype == PTYPE_WEB and app_settings.routable) else False
...
class AutoscaleMixin(AppBaseMixin):
    """Mixin for autoscaling operations."""
    ...
    def autoscale(self, proc_type, autoscale):
        ...
        target = {"apiVersion": "apps/v1", "kind": "Deployment", "metadata": {"name": name}}
        try:
...
class UsageMixin(AppBaseMixin):
    """Mixin for usage reporting."""
...
class StateMixin(AppBaseMixin):
    """Mixin for state synchronization operations."""

    def state_to_k8s(self):
        """Sync app state to Kubernetes."""
        release = Release.latest(self)
        if release is None or release.build is None:
        ...
        for ptype, scale in self.structure.items():
            response = self.scheduler.deployment.get(
                self.id, self.get_deploy_name(ptype), ignore_exception=True
            )
            if response.status_code == 404 and scale > 0:
                ...
                data = response.json()
                self.log(
                    "get deployment status_code {}, message: {}".format(
...
class App(
    ...
    Mixin organization:
    - LifecycleMixin: create, restart
    - DeployMixin: deploy, scale, mount, pipeline, clean
    - RunMixin: one-off command execution
    - KubernetesMixin: pod/deployment/event operations
    - HealthMixin: HTTP/TCP health verification
    - ConfigMixin: configuration and environment management
    - AutoscaleMixin: HPA autoscaling
    ...
    id = models.SlugField(max_length=63, unique=True, null=True, validators=[validate_app_id])
    uid = models.PositiveIntegerField(unique=True, editable=False)
    workspace = models.ForeignKey("Workspace", on_delete=models.CASCADE)
    structure = models.JSONField(default=dict, blank=True, validators=[validate_app_structure])
        ...
        try:
            ...
            try:
                if self.scheduler.ns.get(self.id).status_code == 200:
                    err = "{} already exists as a namespace in this kuberenetes setup".format(
                        self.id
                    )
    ...
    def delete(self, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                raise ServiceUnavailable(
                    "Could not delete Kubernetes Namespace {} " "within 30 seconds".format(self.id)
                ) from e
        except KubeHTTPException:
            pass
