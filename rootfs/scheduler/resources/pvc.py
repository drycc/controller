...
class PersistentVolumeClaim(Resource):
    ...
    def patch(self, namespace, name, **kwargs):
        url = self.api('/namespaces/{}/persistentvolumeclaims/{}', namespace,
                       name)
        data = self.manifest(namespace, name, **kwargs)
        response = self.http_patch(url, json=data, headers={"Content-Type": "application/merge-patch+json"})
        if self.unhealthy(response.status_code):
            self.log(namespace, 'template used: {}'.format(json.dumps(data, indent=4)), logging.DEBUG)
