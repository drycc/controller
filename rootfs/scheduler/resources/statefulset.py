...
class Statefulset(Resource):
    ...
    def get(self, namespace, name=None, ignore_exception=False, **kwargs):
        """
        Fetch a single Statefulsets or a list
        """
        url = '/namespaces/{}/statefulsets'
        args = [namespace]
        if name is not None:
            args.append(name)
            url += '/{}'
            message = 'get Statefulset "{}" in Namespace "{}"'
        else:
            message = 'get Statefulsets in Namespace "{}"'
    ...
    def patch(self, namespace, name, manifest, ignore_exception=False, **kwargs):
        url = self.api("/namespaces/{}/statefulsets/{}", namespace, name)
        ...
        if self.unhealthy(response.status_code):
            ...
            if not ignore_exception:
                raise KubeHTTPException(response, 'patch Statefulset "{}"', name)
        return response
