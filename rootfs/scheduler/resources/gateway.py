...
class Gateway(Resource):
    ...
    def manifest(self, namespace, name, **kwargs):
        ...
        if "allowed_listeners" in kwargs:
            data["spec"]["allowedListeners"] = kwargs.get("allowed_listeners")
...
class ListenerSet(Resource):
    ...
    def get(self, namespace, name=None, ignore_exception=False, **kwargs):
        if name is not None:
            url = self.api("/namespaces/{}/listenersets/{}", namespace, name)
            message = 'get ListenerSet ' + name
        else:
            url = self.api("/namespaces/{}/listenersets", namespace)
            message = 'get ListenerSets'
    ...
    def create(self, namespace, name, ignore_exception=False, **kwargs):
        ...
        response = self.http_post(url, json=data)
        if not ignore_exception and self.unhealthy(response.status_code):
            raise KubeHTTPException(response, "create listenerset {}".format(namespace))
        return response
    ...
    def patch(self, namespace, name, ignore_exception=False, **kwargs):
        ...
        if response.status_code == 409:
            return response
        if not ignore_exception and self.unhealthy(response.status_code):
            raise KubeHTTPException(response, "patch listenerset {}".format(namespace))
        return response
    ...
    def delete(self, namespace, name, ignore_exception=True):
        ...
        response = self.http_delete(url)
        if not ignore_exception and self.unhealthy(response.status_code):
            raise KubeHTTPException(response, 'delete listenerset "{}"', namespace)
        return response
