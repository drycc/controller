...
- name: DRYCC_BUILDER_KEY
  valueFrom:
    secretKeyRef:
      ...
      key: builder-key
{{- if (eq "off-cluster" .Values.global.database_location) }}
...
{{- if (.Values.global.redis_location) and eq "on-cluster" .Values.global.redis_location }}
...
{{- else if (.Values.global.redis_location) and (eq "off-cluster" .Values.global.redis_location) }}
- name: DRYCC_REDIS_ADDRS
  valueFrom:
    secretKeyRef:
      name: redis-creds
...
- name: DRYCC_REDIS_PASSWORD
  valueFrom:
    secretKeyRef:
      name: redis-creds
      key: password
{{- if (.Values.global.influxdb_location) and eq (.Values.global.influxdb_location) "off-cluster" }}
- name: "INFLUXDB_URL"
  valueFrom:
    secretKeyRef:
      name: influxdb-creds
